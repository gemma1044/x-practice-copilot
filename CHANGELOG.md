...
## 2026-09-10 · X 视频封面误判修复

- 修复 X 未播放视频只渲染播放器封面、尚未出现 `<video>` 节点时被误判为“无视频”的问题；页面识别同时兼容 `videoPlayer` 与 `videoComponent` 容器。
- 页面 DOM 的媒体标记降级为提示信息，不再作为下载门禁；用户点击唯一“下载并本地截帧”按钮后，仍由既有本机 bridge 与 `yt-dlp` 根据帖子 URL 作最终判断。
- 新增仅有播放器容器、没有 `<video>` 节点的真机壳回归覆盖；扩展版本升级为 `0.1.10`，不新增入口或数据通道。
