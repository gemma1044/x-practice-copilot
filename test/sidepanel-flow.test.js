...
test("Demo 完成灵感、实践、证据、草稿与本机截帧主路径", { timeout: 30_000 }, async (t) => {
  ...
  await panel.getByRole("button", { name: "改编成我的" }).click();
  assert.equal(await panel.getByRole("button", { name: "生成我的视频 Prompt" }).isDisabled(), true);
  await panel.getByLabel("你想把它改成什么视频？").fill("把产品替换成红色跑鞋，场地换成海边日落");
  await panel.getByRole("button", { name: "生成我的视频 Prompt" }).click();
  await panel.getByText("我的视频 Prompt 已生成。").waitFor();
  assert.match(await panel.locator("#takeaway-prompt").textContent(), /红色跑鞋/u);
  await panel.getByRole("button", { name: "复制我的视频 Prompt" }).click();
  await panel.getByText("我的视频 Prompt 已复制。").waitFor();

  await panel.getByRole("button", { name: "复刻原片" }).click();
  assert.match(await panel.locator("#takeaway-prompt").textContent(), /30 秒竖屏时尚短片/u);
});
